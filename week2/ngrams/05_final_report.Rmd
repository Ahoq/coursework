...
total_count  <- read_csv(here("total_counts.csv"), col_names = c("year", "total_volume", "page_count", "book_count"))
...
new_df <- left_join(year_count, total_count, by = "year") %>% mutate(prop = volume/total_volume)
new_df2 <- new_df  %>% select(term, year, prop, volume) %>% filter(c(term == "1883" | term == "1910" | term == "1950")) %>% mutate(term = factor(term))
...
ggplot(new_df2) + geom_line(aes(x= year, y = prop, color = term )) +
  scale_y_continuous(label = percent) +
  
  labs(x= "Year", y = "Proportion", title = "Figure 3a") +
...
ggplot(new_df2) + geom_line(aes(x= year, y = volume, color = term )) +
  scale_y_continuous(label = percent) +
  
  labs(x= "Year", y = "Volume", title = "Figure 3a") +
...
```{r plot-totals}
new_df %>% group_by(year) %>% ggplot() + geom_line(aes(x = year, y=total_volume), color = "purple") + scale_y_continuous(label = comma) +
  
  labs(x= "Year", y = "Count", title = "1850 - 2012") +
...
new_df1 <- new_df  %>%
  ...
  mutate(peak_year = year) %>%
  mutate(half_prop =  prop/2) %>%
  ungroup()
```
...
new_df <- new_df %>% select(term, year, prop)
new_df1 <- new_df1 %>% select(term, peak_year, half_prop)

df<- merge(new_df, new_df1) %>% 
  subset(term >= 1875 & term <= 1975) %>% 
  subset(year >= peak_year) %>% group_by(term) %>%
  filter(prop < half_prop) %>% 
  subset(year == min(year)) %>% 
  mutate(half_life = year - peak_year)
  
half_lifes <- left_join(new_df, new_df1) %>%
...
half_lifes %>%
  ggplot()+
  geom_jitter(aes(x = term, y = half_life)) +
...
max <- max(half_lifes$half_life)
min <- min(half_lifes$half_life)

half_lifes %>% filter(half_life == min) %>%
  select(term, year, half_life)

half_lifes %>% filter(half_life == max) %>%
  select(term, year, half_life)
